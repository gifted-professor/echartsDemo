<template>
    <div ref="chartOne" style="width: 100%; height: 200px; margin-top: -40px"></div>
</template>
...
let option = {
    title: {
        text: props.value.title.text ? props.value.title.text : '业主',
        textStyle: {
            fontSize: 14,
            color: '#6495ED',
            lineHeight: 20
    ...
    legend: {
        ...
        top: props.value.legend ? props.value.legend.top : '15%',
        radius: [60, 70],
        itemGap: 5, //标签高度
        selectedMode: false,
        // icon: 'pin',
        data: props.value.nameList,
        textStyle: {
            color: '#fff',
            rich: {
                uname: {
    ...
    color: props.value.chartColor ? props.value.chartColor : ['#53C8D1', '#59CB74', '#FBD444', '#7F6AAD', '#585247'],
    series: [
        {
            name: '',
            type: 'pie',
